  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2">
        {posts.map((p:any)=>(
          <a key={p.id} href={p.external_url || '#'} target="_blank" className="block aspect-square rounded overflow-hidden bg-neutral-100">
            {p.cover_url ? <img src={p.cover_url} alt="" className="w-full h-full object-cover"/> : null}
          </a>
        ))}
        {posts.length === 0 && (
          <div className="col-span-3 text-sm text-neutral-500">Sin publicaciones aún.</div>
        )}
      </div>
    </div>
  );
